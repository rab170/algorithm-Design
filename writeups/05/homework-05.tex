...
\\ \\ \\ \\ \\ \\ \\ \\ \\ \\
Clearly OPT' is more like EDF. In order to prove that it still produces an acceptable output, consider the following four cases. Also note that by the definition of EDF, we know that the deadline $d_{\alpha(k)}$ is earlier than (or tied with) the deadline $d_{\beta(k)}$. Stated another way: $d_{\alpha(k)}$ $\leq$ $d_{\beta(k)}$
\\ \\
...
>>>>>>> 4d9a598012475e5dcd010334a772dba3f7331643
\\ \\
Let 
