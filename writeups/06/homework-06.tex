...
\begin{homeworkProblem} 
\textbf{(b)} \\
\textbf{Input: A linear graph of 2 nodes (ie, two nodes connected by one edge) and set of $k$ packets $p_1, p_2, ..., p_k$ where each packet $p_i$ consists of $(A, r_p, s_p, t_p)$ where $A$ is the packet name, $r_p$ is the packet release time, $s_p$ is the packet start node, and $t_p$ is the packet's target node.}
\\
\textbf{Output: The order of "sent" packets for each of the $n$ nodes such that the network is cleared of all packets as quickly as possible.} 
\\ \\
\textbf{Theorem:} 
\\
\textbf{Proof:} 
