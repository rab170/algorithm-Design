...
\If{$w_{L_i} \le (L - K'_{i-1})$}
	\If{$w_{L_i}$ is the only word on the last line}
		...
		\State $P_i$ changed from $L - w_i$ to 0 ($\Delta P < 0$)
		\State $P_{i-1} -= w_i$
	\Else
	...
	\EndIf
\Else
	\State No move takes place
	\State $\Delta P = 0$
\EndIf 
\end{algorithmic}
\leavevmode
\\
We know that OPT' can fit $w_k$ on the prior line, since G made this selection. Clearly, this move alone will have no effect on P (P gains $w_k$ on one line, and loses it on the next). Clearly, this makes OPT' more like G. 
